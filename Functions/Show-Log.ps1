function Show-Log($filePath)
{
    ...
    $datetime  = [datetime]::Now.ToString("HHmm")
    ...
    $resultCount = Search-UnifiedAuditLog -EndDate:$EndDate -StartDate:$StartDate -ResultSize:1 -UserIds:$Global:userIds |
        Out-String -Stream |
        Select-String -InputObject:"ResultCount"
    ...
    {	
        ...
        if ($results)
        {
            ...
            Write-Output "$recordType - $number" | Out-File $filePath -Append
        }
    }
    ...
    if($resultCount)
    {
        $numbertotal =$resultCount.tostring().split(":")[1]
        Write-Host "--------------------------------------"
